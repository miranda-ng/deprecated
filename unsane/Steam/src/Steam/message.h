...
namespace SteamWebApi
{
	...
	class SendMessageRequest : public HttpsGetRequest
	{
	public:
		SendMessageRequest(const char *token, const char *umqId, const char *steamId, const char *text) :
			HttpsGetRequest(STEAM_API_URL "/ISteamWebUserPresenceOAuth/Message/v0001")
		{
			AddParameter("jsonp=jQuery111107116868502926081_%lld", time(NULL));
			AddParameter("access_token", token);
			AddParameter("umqid", umqId);
			AddParameter("steamid_dst", steamId);
			AddParameter("type=saytext");
			AddParameter("text", ptrA(mir_urlEncode(text)));
		}
	};
}
