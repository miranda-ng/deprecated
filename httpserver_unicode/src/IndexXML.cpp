...
bool bCreateIndexXML(const TCHAR *pszRealPath, const TCHAR *pszIndexPath, const TCHAR *pszSrvPath, DWORD dwRemoteIP)
{
	...
	// Write Header
	WriteFile(hFile, szXmlHeader1, SIZEOF(szXmlHeader1) * sizeof(TCHAR), &dwBytesWritten, NULL);
	...
	WriteFile(hFile, szXmlHeader2, SIZEOF(szXmlHeader2) * sizeof(TCHAR), &dwBytesWritten, NULL);
	...
	WriteFile(hFile, szXmlTail, SIZEOF(szXmlTail) * sizeof(TCHAR), &dwBytesWritten, NULL);
